...
func (db *balancesDB) QueryBalancesByToAddres(address *common.Address) (*Balances, error) {
	...
	err := db.gorm.Table("balances").Where("address", address.String()).Take(&balanceEntry).Error
	if err != nil {
...
func (db *balancesDB) QueryHotWalletBalance(address, tokenAddress common.Address) (*Balances, error) {
	var balanceEntry Balances
	err := db.gorm.Table("balances").Where("address = ? and token_address = ?", address.String(), tokenAddress.String()).Take(&balanceEntry).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
...
func (db *balancesDB) UpdateOrCreate(balanceList []TokenBalance) error {
	...
	err := db.gorm.Table("balances").Where("address_type = ?", 1).Take(hotWalletBalance).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			log.Info("Hot wallet is not config, please config it")
			return nil
		}
		return err
	}
	for _, value := range balanceList {
		...
		err := db.gorm.Table("balances").Where("address = ? and token_address = ? and address_type = ?", value.Address, value.TokenAddress, 0).Take(userBalanceEntry).Error
